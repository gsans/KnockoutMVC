...
using KnockoutApplication.Mvc.Models;

namespace KnockoutApplication.Mvc.Controllers
{
    public class TasksController : ApiController
    {
        private TasksModelContainer db = new TasksModelContainer();

        // GET api/Tasks
        public IEnumerable<Task> GetTasks()
        {
            return db.Tasks1.AsEnumerable();
        }

        // GET api/Tasks/5
        public Task GetTask(int id)
        {
            Task task = db.Tasks1.Find(id);
            if (task == null)
            {
